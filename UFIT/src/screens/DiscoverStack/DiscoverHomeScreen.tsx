...
export default function DiscoverHomeScreen({
  ...
  const filteredPrograms = filter
    ? programs.filter(
        (program) =>
          program.programCategory.toLowerCase() === filter.toLowerCase()
      )
    : programs;
  ...
  React.useEffect(() => {
    API.getPrograms()
      .then((response) => {
        //works?
        let noOriginalPrograms = response.data.data.filter(
          (program: Program) => !program.originalProgramId
        );
    ...
    <SafeAreaView>
      <View style={FeedStyles.viewContainer}>
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <Text style={FeedStyles.titleBarText}>Discover</Text>
          <TouchableOpacity
        ...
        >
          {filteredPrograms.map((program, index) => (
            <Pressable
              key={index}
              onPress={() => navigator.navigate("Program", { program })}
              style={{
                shadowColor: "#171717",
            ...
            >
              ...
              >
                ...
                >
                  <Text
                    style={[FeedStyles.programDescription, { height: "60%" }]}
                  >
                  ...
                  >
                    ...
                    {program.isCreatedByAI && (
                      ...
                      >
                        <FontAwesome5 name="brain" size={20} color="white" /> AI
                        Generated
                      </Text>
                    )}
                  </View>
