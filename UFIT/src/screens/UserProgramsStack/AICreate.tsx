...
export default function AICreate({ navigation }: AICreateScreenProps) {
  ...
  const { addProgram } = useUserPrograms(); //UserProgramsContext hook
  ...
  //HANDLE SESSIONS and CREATE MOVEMENTS are COPIED FROM RICARDO'S CODE IN PROGRMCREATE.TSX
  ...
  return (
    <LinearGradient
      top={
        aiProgram
          ? getGradientColors(aiProgram.programCategory.toLowerCase())[0]
          : "#000"
      }
      bottom={
        aiProgram
          ? getGradientColors(aiProgram.programCategory.toLowerCase())[1]
          : "#EA9CFD"
      }
      style={{ minHeight: "100%" }}
    >
      <SafeAreaView>
        <View style={discoverProgramStyles.viewContainer}>
          <Text
            style={[
              programStyles.titleBarText,
              { color: "white", alignSelf: "center", paddingLeft: 0 },
            ]}
          >
            AI Create
          </Text>
          ...
          >
            ...
            >
              <View
                style={[
                  trackingStyles.timerButton,
                  { width: 200, backgroundColor: "rgba(0,0,0,0.3)" },
                ]}
              >
                <Text
          ...
          {aiProgram && (
            <ScrollView style={trackingStyles.sessionsContainer}>
              ...
              <Text
                style={[
                  discoverProgramStyles.theProgramTitle,
                  { fontWeight: "normal" },
                ]}
              >
                    ...
                    {session.movements.map((movement: any, index: number) => (
                      ...
                      >
                        ...
                        {movement.typeTracking.trackingType === "setsreps" && (
                          <Text style={discoverProgramStyles.movementText}>
                            {movement.typeTracking.sets} x{" "}
                            {movement.typeTracking.reps}
                          </Text>
                        )}
